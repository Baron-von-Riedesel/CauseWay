...
;
	...
	;
	pushad
	mov	eax,decode_c_struc.DecC_Len[esi]
	mov	StatusTotalLength,eax
	mov	StatusNoBar,2
	call	ShowProgress
	popad
...
;
	mov	ErrorNumber,2
	movzx	ecx,[NewHeader.NewSegments]
...
;
@@NoRelocs:	mov	ecx,[NewHeader.NewExports]
	or	ecx,ecx
	jz	@@NoExports
	mov	ErrorNumber,2
	call	Malloc
...
;
@@NoExports:	mov	ecx,[NewHeader.NewImports]
	or	ecx,ecx
	jz	@@NoImports
	mov	ErrorNumber,2
	call	Malloc
...
DecodeFile	proc	near
	...
	;
	mov	eax,d[decode_c_struc.DecC_ID+esi]
	and	eax,0FFFFFFh
	cmp	eax,"CWC"
	jnz	@@form_error
	movzx	ecx,b[decode_c_struc.DecC_Bits+esi]
	push	ds
	mov	ds,CodeSegAlias
	assume ds:_TEXT
	mov	b[@@Shifter+2],cl
	...
	assume ds:DGROUP
	pop	ds
	mov	ebx,decode_c_struc.DecC_Len[esi]
	mov	_CD_LastLen,ebx
;
