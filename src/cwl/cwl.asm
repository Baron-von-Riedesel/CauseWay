...
Write3PFile	proc	near
	...
	mov	NewHeaderStruc.NewSize[ebp],size NewHeaderStruc ;update 3P size.
	;
	...
	inc	eax
	mov	NewHeaderStruc.NewAutoDS[ebp],ax
	;
@@NoDGROUP:	;Set entry SS:ESP values.
	;
	mov	eax,ProgramEntrySS
	mov	NewHeaderStruc.NewEntrySS[ebp],ax
	mov	eax,ProgramEntryESP
	mov	NewHeaderStruc.NewEntryESP[ebp],eax
	;
	...
	cmp	LSeg.SPage[eax],0	;32-bit segment?
	jnz	@@32Bit
	or	NewHeaderStruc.NewFlags[ebp],(1 shl 14)+1
	;
@@32Bit:	;Now set entry CS:EIP values.
	;
	mov	eax,ProgramEntryCS
	mov	NewHeaderStruc.NewEntryCS[ebp],ax
	mov	eax,ProgramEntryEIP
	mov	NewHeaderStruc.NewEntryEIP[ebp],eax
	;
	...
	jnz	@@NotNear0
	inc	ProgramEntryCS
	inc	NewHeaderStruc.NewEntryCS[ebp]
	or	NEARModel,-1
;
...
@@NotNear0:	mov	esi,SEGDEFList
	...
	jz	@@NotNear1
	inc	eax
@@NotNear1:	mov	NewHeaderStruc.NewSegments[ebp],ax
	shl	eax,3		;8 bytes per entry.
	add	NewHeaderStruc.NewSize[ebp],eax	;update 3P size.
	;
	...
	jz	@@0
	mov	eax,[esi]
	mov	NewHeaderStruc.NewRelocs[ebp],eax
	shl	eax,2		;4 bytes per entry.
	add	NewHeaderStruc.NewSize[ebp],eax
	;
...
@@1:	mov	edi,[esi]		;point to this SEGDEF.
	...
	dec	ecx
	jnz	@@1
	add	NewHeaderStruc.NewSize[ebp],edx	;update 3P size.
	mov	NewHeaderStruc.NewLength[ebp],edx	;set EXE image size.
	mov	NewHeaderStruc.NewAlloc[ebp],edx	;set memory size.
;
...
@@12:	mov	edi,[esi]
	...
	add	edx,eax
@@11:	
	mov	NewHeaderStruc.NewExports[ebp],edx
	add	NewHeaderStruc.NewSize[ebp],edx	;update 3P size.
;
...
;
	...
	mov	NewHeaderStruc.NewImports[ebp],eax
	add	NewHeaderStruc.NewSize[ebp],eax
	mov	eax,IMPORTModules
	mov	eax,[eax]
	mov	NewHeaderStruc.NewImportModCnt[ebp],eax	;store module count.
;
