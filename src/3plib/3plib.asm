...
Copyright	db 13,10
	db 79 dup ("Ä"),13,10
	db "3PLib v2.00 Public Domain.",13,10
	db 79 dup ("Ä"),13,10
	db 13,10
	db "$"
...
@@0:	;Read header so we can figure out the format.
	...
	;Check the format.
	;
	cmp	w[edx].NewHeaderStruc.NewID,"P3"
	jz	@@2
	cmp	w[edx].NewHeaderStruc.NewID,"ZM"
	mov	ErrorNumber,5
	jnz	@@9
	;
...
@@2:
	...
	;
	cmp	[edi].NewHeaderStruc.NewExports,0
	mov	ErrorNumber,6
	jz	@@9
	;
	...
	;
	movzx	edx,[edi].NewHeaderStruc.NewSegments
	shl	edx,3
	sys	cwcInfo
	jc	@@3p0
	mov	edx,eax
@@3p0:	mov	cx,dx
	...
	;Skip relocations.
	;
	mov	edx,[edi].NewHeaderStruc.NewRelocs
	shl	edx,2
	or	edx,edx
	jz	@@3p1
	sys	cwcInfo
...
@@3p1:	mov	cx,dx
	...
	;Load export details.
	;
	mov	ecx,[edi].NewHeaderStruc.NewExports
	sys	GetMemLinear32
	mov	ErrorNumber,7
	jc	@@9
	mov	edx,ecx
